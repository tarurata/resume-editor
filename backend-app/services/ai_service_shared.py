...
class AIServiceShared:
    ...
    def _initialize_llm_client(self):
        """Initialize LLM client using existing configuration"""
        try:
            # We'll make HTTP calls to the frontend LLM service
            # Or we can create a shared Python LLM client
            logger.info("Shared LLM service initialized")
        except Exception as e:
            logger.warning(f"Could not initialize shared LLM service: {e}")
    
    async def _call_llm(self, prompt: str, params: Dict[str, Any] = None) -> str:
        """Call the existing LLM service"""
        try:
            # Option 1: Call frontend LLM service via HTTP
            # This would require creating an API endpoint in the frontend
            # For now, we'll use a direct approach
            
            # Option 2: Use the same OpenAI client as the frontend
            import openai
            ...
            response = await client.chat.completions.create(
                ...
                max_tokens=params.get('maxTokens', 1000) if params else 1000,
...
Extract personal information:"""
        ...
        try:
            response = await self._call_llm(prompt, {'temperature': 0.1, 'maxTokens': 500})
...
Extract sections:"""
        ...
        try:
            response = await self._call_llm(prompt, {'temperature': 0.1, 'maxTokens': 2000})
...
Extract structured data:"""
        ...
        try:
            response = await self._call_llm(prompt, {'temperature': 0.1, 'maxTokens': 3000})
    ...
    async def improve_content(self, content: str, improvement_type: str = "general", context: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            response = await self._call_llm(prompt, {'temperature': 0.7, 'maxTokens': 1000})
            
            # Generate suggestions
            suggestions_prompt = f"Provide 3-5 specific suggestions to improve this resume content:\n\n{content}"
            suggestions_response = await self._call_llm(suggestions_prompt, {'temperature': 0.6, 'maxTokens': 500})
